...
public class AddCSV {
	...
	public static void main( String[] args ) throws Exception
	{
		...
		// csv file seperated by '`'
		directoryToCSV.put("/media/wind/Data/郝阳-上市公司数据/2014",   "2014.csv");
...
//		directoryToCSV.put("F:\\郝阳\\Done\\中小板2012年年度财务报告", "2012.csv");
		...
		for (String dirPath: directoryToCSV.keySet())
		{
			...
			csvPath = "/media/wind/Data/郝阳-上市公司数据/CSV Template/" + directoryToCSV.get(dirPath);
			...
			resultPath = "/media/wind/Data/郝阳-上市公司数据/CSV Template/" + "out_" + directoryToCSV.get(dirPath);
	...
	private static void buildCSV(String csvPath, String featurePath, String partPath, String codePath, String resultPath) throws IOException {
		...
		System.out.println("Build features complete.");
		fileNameToPart = buildFileNameMap(partPath);
		System.out.println("Build parts complete.");
		codeToFileName = buildCodeMap(codePath);
		System.out.println("Build codes complete.");
		...
		System.out.println("Total " + firmLines.size() + " firm records.");
		...
		System.out.println("Total " + legalCodes.keySet().size() + " legal firm records.");
		...
		System.out.println("Total " + firmLines.size() + " firm records. (2)");
		...
		try
		{
			for (String item: firmLines)
			{
				...
				if (words.length >= 22)
				{
					...
					else
					{
						...
						if (features.size() != 10)
						{
							System.out.println("features not enough in " + fileName + ".");
							for (int i=0;i<10;i++)
								features.add("");
						}
					}
			...
			System.out.println("Write " + writeCount + " lines of records.");
